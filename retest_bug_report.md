...
- **Reverification (2026-09-16, production): ✅ LIKELY FIXED, APP-WIDE.** Confirmed live across every module spot-checked -- `/unit`, `/dashboard/graphical`, `/settings/driver`, `/tracking`, and `/reports/standard` -- both a raw direct `page.goto()` AND an in-place `page.reload()` now correctly stay on the target URL instead of bouncing to `/home`. This was caught while triaging two specific test failures (`test_rep_cus_017_refresh_during_creation`, `test_rep_page_refresh_mid_workflow_recovers`) that were written to assert NEW-1's broken behavior and started failing because the redirect no longer happens. **This is a major finding given NEW-1's footprint** -- dozens of regression pins across nearly every module in this suite were written specifically to assert the old broken (redirect) behavior, and none of those have been updated in this pass; only the two tests directly encountered during this triage were fixed. A dedicated follow-up pass to find and flip every NEW-1-dependent regression pin across the whole suite (Dashboard, Unit, Settings, VT, Admin Panel, etc.) is recommended before treating NEW-1 as fully closed out in the test suite itself, even though the underlying application bug now appears resolved.
